...
const ProductListItem = ({ product, onClick, selectedProduct }: Props) => {
  const { name, brand, category, variants, image } = product;
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        "w-full h-20 lg:h-16 px-3 py-2.5 bg-white border border-border flex flex-row items-center gap-3 active:bg-gray-50 transition-colors text-left hover:bg-muted/50 rounded-lg",
